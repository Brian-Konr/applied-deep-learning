...
def main():
    ...
    if args.dataset_name is not None:
        ...
        # preprocess for true answer start
        def count_true_answer_start(example):
            ...
            for paragraph_id in example["paragraphs"]:
                if paragraph_id != example["relevant"]:
                    start += len(paragraph_ref_map[paragraph_id])
                else:
                    start += ans_start
                    break
            example["answer"]["start"] = start
            return example
        raw_datasets = raw_datasets.map(count_true_answer_start)
        print(f"preprocess for true answer start after loading dataset done!!")
    ...
    def prepare_train_features(examples):
        ...
        for paragraphs in examples["paragraphs"]:
            ...
            for paragraph_id in paragraphs:
                context += paragraph_ref_map[paragraph_id]
